...
List<ChatMessage> messages = [
  ...
  ChatMessage(
    messageContent: "terrible. Life has been so unkind lately. I really feel defeated.",
    messageType: "sender",
  ),
  ChatMessage(
    messageContent: "You sound really low. I have something for you. Care to listen?",
    messageType: "receiver",
  ),
  ChatMessage(
    messageContent: "Cool",
                    ...
                    FloatingActionButton(
                      onPressed: () async {
                        ...
                        if (sentiment == "positive") {
                          ...
                          ChatMessage newMessage = ChatMessage(messageContent: positiveResponses[i], messageType: "receiver");
                          List<ChatMessage> newMessages = [...messages, newMessage];
                          if (states.sentimentDetails[states.sentimentDetails.length - 1] == "negative") {
                            states.moveToActivity!();
                          }
                          states.sentimentDetails = [...states.sentimentDetails, "positive"];
                          setState(() {
                            messages = newMessages;
                          });
                        } else if (sentiment == "negative") {
                          var i = random.nextInt(negativeResponses.length);
                          ChatMessage newMessage = ChatMessage(messageContent: negativeResponses[i], messageType: "receiver");
                          List<ChatMessage> newMessages = [...messages, newMessage];
                          if (states.sentimentDetails[states.sentimentDetails.length - 1] == "negative") {
                            states.moveToActivity!();
                          }
                          states.sentimentDetails = [...states.sentimentDetails, "negative"];
                          setState(() {
                            messages = newMessages;
                          });
                        } else if (sentiment == "neutral") {
                          var i = random.nextInt(neutralResponses.length);
                          ChatMessage newMessage = ChatMessage(messageContent: neutralResponses[i], messageType: "receiver");
                          List<ChatMessage> newMessages = [...messages, newMessage];
                          states.sentimentDetails = [...states.sentimentDetails, "neutral"];
                          setState(() {
                            messages = newMessages;
                          });
                        }
